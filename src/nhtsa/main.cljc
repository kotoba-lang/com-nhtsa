(ns nhtsa.main
  "Kotodama WASM entrypoint for the Nhtsa clean-room actor (L4) — Clojure port.
  ...
  facts (`nhtsa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"nhtsa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nhtsa-compat" :tier tier :entities entities} 200])
